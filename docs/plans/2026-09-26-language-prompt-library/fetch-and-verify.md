...
- The base address is one constant in the app, next to `MODEL_BASE_URL` in the same style.
...
- `<app data>/language-presets/<id>/preset.md` plus `meta.json` with `id`, `version`, `sha256`
  and the download date. One copy per id, shared by every language that uses it (en-GB and
  en-US use one `english` file).
- `<app data>/language-presets/index.json` is the cached index, with the `ETag` of the last
  response; the next fetch sends `If-None-Match`, so an unchanged index costs one small
  response.
- The app re-validates a local file when it reads it (hash against `meta.json`); a damaged file
  falls back to the language's built-in default and shows "Download again".
...
- For "update available": at most once a day, only while at least one language uses a
  downloaded preset, and only while **Settings → General → Check for preset updates** is on (on
  by default, one switch to turn off).
...
- The list uses the cached index and says "Offline: showing the list from <date>"; presets not
  yet downloaded show a disabled Download button. With no cache, the sheet says the list needs
  a connection and the built-in default stays in use.
