...
| Card fields: `id`, `name`, `connection`, `address`, `model`, `needs_key`, `runs`, `cost`, optional `languages` and `notes`; AI cards may add `thinking_off` | The table columns a reader compares; `connection` must be one the code has. `thinking_off` shows on the card (GitHub renders front matter as a table) but not in the wide services table. |
| Front matter values must also be valid YAML | GitHub renders the front matter; an invalid value would show an error box instead. |
| The language preset catalogue is generated from `index.json` by the same script | One command regenerates every table; the preset validator stays unchanged. |
| Model suggestions live in one page, `models.md`, as starting points | Hardware and language advice in one place; the thinking-models page links there instead of repeating it. |
...
| The app's "this Mac" strings become "this computer"; the Built-in presets are named "Built-in (on-device)", and a stored "Built-in (this Mac)" name is renamed on load; preset ids and legacy template names are unchanged | The words are user-facing; the ids and legacy names are compared by config migrations. |
...
scripts/docs-cards.mjs             validates cards, writes the generated tables and the catalogue
presets/languages/README.md        generated catalogue grouped by language, then the preset guide
```
